// Данный скрипт быстрее всего работает в FireFox
// Скопировать скрипт в буфер обмена (можно прямо с этими комментариями)
// Открыть страницу линкдин со скилами (внизу скилов нажать Show all), прокрутить вниз страницы до конца, ЧТОБЫ ВСЕ СКИЛЫ ПРОГРУЗИЛИСЬ, открыть DevTools (клавиша F12, или Command (⌘) + Option (⌥) + J) и вкладку Console.
...
// Ауф, я подвердил твои скилы через кнопку "Endorse", если будет время, подтверди и мои. Буду премного благодарен. Если лень нажимать, вот скрипт для автоматизации данного процесса через браузер. Делись скриптом с другими волками.
